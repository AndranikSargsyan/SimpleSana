...
from sana.model.builder import MODELS
from sana.model.nets.basic_modules import GLUMBConv
from sana.model.nets.sana import Sana, get_2d_sincos_pos_embed
from sana.model.nets.sana_blocks import (
    CaptionEmbedder,
    LiteLA,
...
from sana.model.utils import auto_grad_checkpoint
...
class SanaMS(Sana):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, return_logvar=False, jvp=False, **kwargs):
        ...
        if mask is not None:
            ...
            mask = mask.repeat(y.shape[0] // mask.shape[0], *(1, ) * (len(mask.shape) - 1)) if mask.shape[0] != y.shape[0] else mask
